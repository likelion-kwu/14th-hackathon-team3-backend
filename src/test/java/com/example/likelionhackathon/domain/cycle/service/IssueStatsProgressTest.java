...
class IssueStatsProgressTest {
    ...
    void countsFinishedIssuesFully() {
        assertThat(new IssueStats(4, 2, 2, 0, 0, 0, 0).progressRate()).isEqualTo(50);
    }

    @Test
    void countsPartiallyCheckedIssues() {
        // 완료 1건 + 완료 조건 7/12(0.58)를 채운 진행 중 1건 → (1 + 0.58) / 4
        IssueStats stats = new IssueStats(4, 1, 3, 0, 0, 0, 7.0 / 12);
    ...
    void ignoresCanceledIssuesInTheDenominator() {
        // 전체 5건 중 1건 취소 → 분모는 4
        assertThat(new IssueStats(4, 2, 2, 0, 0, 1, 0).progressRate()).isEqualTo(50);
    }

    @Test
    void reachesHundredOnlyWhenEveryIssueIsDone() {
        assertThat(new IssueStats(3, 3, 0, 0, 0, 0, 0).progressRate()).isEqualTo(100);
    }

    @Test
    void stopsAtNinetyNineWhileAnythingIsUnfinished() {
        // 완료 조건을 모두 채웠어도 이슈가 완료 처리되기 전에는 100 이 아니다.
        IssueStats stats = new IssueStats(3, 2, 1, 0, 0, 0, 1.0);
    ...
    void roundsDownSoRemainingWorkNeverLooksComplete() {
        ...
        assertThat(new IssueStats(200, 199, 1, 0, 0, 0, 0).progressRate()).isEqualTo(99);
    }
}
